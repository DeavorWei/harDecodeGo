...
1. **编译程序**：运行 `build.bat` 生成 `bin/har-decode.exe`
2. **放置文件**：将 `har-decode.exe` 复制到包含 `.har` 文件的目录中
3. **双击运行**：双击 `har-decode.exe` 即可自动处理

**运行效果**：
- 程序会自动搜索当前目录下的所有 `.har` 文件
- 输出目录为 `{程序目录}/decode/{har文件名}/`
- 处理完成后按 Enter 键退出

**注意事项**：
- 双击模式使用默认配置（4个并发worker，遇到错误继续处理）
- 如需自定义参数，请使用命令行模式
